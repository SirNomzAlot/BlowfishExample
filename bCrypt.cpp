...
uint32_t f (Keys* key, uint32_t x) {
	uint32_t hold = key->S0[x>>24] + key->S1[x>>16 & 255];
	return ( hold^key->S2[x>>8 & 255])+key->S3[x&255];
}

uint8_t* swap(uint8_t block[8]) {
	uint8_t hold[4];
	hold[0] = block[4];
	hold[1] = block[5];
	hold[2] = block[6];
...
uint32_t block32(uint8_t block[8], bool high) {
	int var = (high) ? 0 : 4;
	return (block[var]<<24)+(block[var+1]<<16)+(block[var+2]<<8)+(block[var+3]);
}
...
uint8_t* encrypt(Keys *key, uint8_t block[8]) {
	...
	for (short r = 0; r<16; r++) {
		...
		block[2]^=(key->P[r]>>8)&0xFF;
		block[3]^=key->P[r]&0xFF;

		uint32_t hold = f(key, block32(block, false));

		block[4]^=hold>>24;
		block[5]^=(hold>>16)&0xFF;
		block[6]^=(hold>>8)&0xFF;
		block[7]^=hold&0xFF;
	...
	block[4]^=key->P[16]>>24;
	block[5]^=(key->P[16]>>16)&0xFF;
	block[6]^=(key->P[16]>>8)&0xFF;
	block[7]^=key->P[16]&0xFF;

	block[0]^=key->P[17]>>24;
	block[1]^=(key->P[17]>>16)&0xFF;
	block[2]^=(key->P[17]>>8)&0xFF;
...
uint8_t* decrypt(Keys *key, uint8_t block[8]) {
	...
	block[4]^=key->P[1]>>24;
	block[5]^=(key->P[1]>>16)&0xFF;
	block[6]^=(key->P[1]>>8)&0xFF;
	block[7]^=key->P[1]&0xFF;

	block[0]^=key->P[0]>>24;
	block[1]^=(key->P[0]>>16)&0xFF;
	block[2]^=(key->P[0]>>8)&0xFF;
	block[3]^=key->P[0]&0xFF;
